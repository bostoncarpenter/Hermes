...
[ -f "$ROOT/spine/.env" ] || cp "$ROOT/spine/.env.example" "$ROOT/spine/.env"

echo "==> Spine stack"
docker compose -f "$ROOT/spine/docker-compose.yml" --env-file "$ROOT/spine/.env" up -d || \
  echo "!! fill in spine/.env first (POSTGRES_PASSWORD, JWT_SECRET, GOOGLE_API_KEY)"

cat <<'EOF'

Done. Next steps:
  1. Fill in spine/.env (POSTGRES_PASSWORD, JWT_SECRET, GOOGLE_API_KEY),
